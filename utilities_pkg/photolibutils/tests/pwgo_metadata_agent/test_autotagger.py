...
from ...pwgo_metadata_agent.pwgo_image import PiwigoImage
from ...pwgo_metadata_agent.constants import Constants
from ...pwgo_metadata_agent.rekognition import RekognitionClient
...
class TestAutotagger:
    ...
    @pytest.mark.asyncio
    async def test_add_tags(self, test_db):
    ...
    @pytest.mark.asyncio
    async def test_add_implicit_tags(self, test_db):
        ...
        verify that 'holdidays' tag was added"""
        img_id = 22
        async with test_db.acquire_dict_cursor(db=Constants.PWGO_DB) as (cur,_):
            sql = """
    ...
    async def test_add_indexed_image(self, mck_rek, test_db):
        ...
        async with test_db.acquire_dict_cursor(db=Constants.REKOGNITION_DB) as (cur,_):
            sql = """
    ...
    @patch.object(AutoTagger, "_get_rek_client")
    async def test_get_face_image_files(self, mck_rek, mck_get_crop, test_db):
        ...
        mck_rek.return_value = AsyncMock(spec=RekognitionClient)
        mck_rek.return_value.detect_faces.return_value = mck_faces
        img = await PiwigoImage.create(img_id)
        ...
        assert mck_get_crop.call_count == 2
    ...
    @pytest.mark.asyncio
    @patch.object(AutoTagger, "_get_rek_client")
    async def test_get_matched_face(self, mck_rek, test_db):
        ...
        mck_rek.return_value = AsyncMock(spec=RekognitionClient)
        mck_rek.return_value.match_face_from_image.return_value = mck_matched_face
        img_id = 826
        img_face_idx = 0
        matched_img_id = 582
        async with test_db.acquire_dict_cursor(db=Constants.REKOGNITION_DB) as (cur,conn):
    ...
    @pytest.mark.asyncio
    @patch.object(AutoTagger, "_get_rek_client")
    async def test_fetch_image_labels(self, mck_rek, test_db):
        ...
        mck_rek.return_value = AsyncMock(spec=RekognitionClient)
        mck_rek.return_value.detect_labels.return_value = mck_matched_labels
    ...
    @pytest.mark.asyncio
    async def test_move_image_to_processed(self, test_db):
        ...
        async with test_db.acquire_dict_cursor(db=Constants.PWGO_DB) as (cur,conn):
            sql = """
                ...
                VALUES (%s, %s)
            """
            await cur.execute(sql, (img_id, Constants.AUTO_TAG_ALB))
            await conn.commit()
            ...
            await cur.execute(sql % (img_id, Constants.AUTO_TAG_PROC_ALB))
            result = await cur.fetchall()
            assert len(result) == 1

    @pytest.mark.asyncio
    @patch('photolibutils.pwgo_metadata_agent.autotagger.RekognitionClient')
    async def test_remove_indexed_faces(self, rek, test_db):
    ...
    @patch('photolibutils.pwgo_metadata_agent.autotagger.RekognitionClient')
    @patch('photolibutils.pwgo_metadata_agent.autotagger.Constants')
    async def test_sync_face_index(self, const, rek, at_create, at_rem_idx, _, test_db):
        """tests the basic functioning of the sync_face_index static method."""
        const.FACE_IDX_ALBS = [129,130,131]
        const.PWGO_DB = Constants.PWGO_DB
        const.DRY_RUN = False
    ...
    @pytest.mark.asyncio
    async def test_process_new_tag(self, test_db):
    ...
    async def test_autotag_image(self, at_get_img_tags, at_get_label_tags, at_get_files, at_add, at_mv, test_db):
        """tests the basic functioning of the main autotag_image method"""
        mck_img_file = MagicMock(spec=IOBase)
        at_get_files.return_value = [(mck_img_file, 0)]
        at_get_img_tags.return_value = [45]
        at_get_label_tags.return_value = [20,21]
        ...
        at_get_img_tags.assert_awaited_once_with(mck_img_file, 0)
        at_get_label_tags.assert_awaited_once()
        at_add.assert_awaited_once_with({45,20,21})
        at_mv.assert_awaited_once()
