...
class AutoTagger():
    ...
    async def autotag_image(self) -> None:
        ...
        logger.debug("beginning autotagging of %s", self.image.file)
    ...
    async def add_implicit_tags(self) -> None:
        ...
        async with DbConnectionPool.get().acquire_dict_cursor(db=Constants.PWGO_DB) as (cnt_cur,_):
            sql = """
                SELECT COUNT(*) AS cnt
                FROM image_tag it
            ...
            if cnt:
                logger.info("adding %s implicit tags for %s", cnt, self.image.file)
                if not Constants.DRY_RUN:
                    async with DbConnectionPool.get().acquire_dict_cursor(db=Constants.PWGO_DB) as (ins_cur,conn):
                        sql = """
    ...
    async def add_tags(self, tags: List[int]) -> None:
        ...
        if tags:
            logger.info("Adding %s tags to %s", tags, self.image.file)
            if not Constants.DRY_RUN:
                async with DbConnectionPool.get().acquire_dict_cursor(db=Constants.PWGO_DB) as (cur,conn):
                    sql = """
                        INSERT INTO image_tag (image_id, tag_id)
                        VALUES (%s, %s)
    ...
    async def add_indexed_image(self, index_cat_id):
        ...
        logger.info(strings.LOG_ADD_IMG_FACES(self.image.file))
        ...
        if not Constants.DRY_RUN:
            ...
            """
            if faces:
                async with DbConnectionPool.get().acquire_dict_cursor(db=Constants.REKOGNITION_DB) as (cur,conn):
                    for frec in faces:
                        face = frec["Face"]
    ...
    async def _get_face_image_files(self):
        ...
        logger.debug(strings.LOG_DETECT_IMG_FACES(self.image.file))
        results = []
        if not Constants.DRY_RUN:
            with self.image.open_file() as img_file:
                client = await self._get_rek_client()
    ...
    async def _move_image_to_processed(self) -> None:
        ...
        logger.info(strings.LOG_MOVE_IMG(self.image.file))
        if not Constants.DRY_RUN:
            async with DbConnectionPool.get().acquire_dict_cursor(db=Constants.PWGO_DB) as (cur,conn):
                ins_sql = """
    ...
    @staticmethod
    async def _get_tags_for_match(face: Dict) -> List[int]:
    ...
    @staticmethod
    async def _get_tag_ids_for_labels(labels: List[str]) -> List[int]:
